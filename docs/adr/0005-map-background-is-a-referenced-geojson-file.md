...
- **The validator is strict.** A `FeatureCollection` of exactly those shapes; an unknown kind or geometry type is rejected, not ignored. A new feature kind is a schema change.
...
- **Fidelity bar: stylised but geo-registered.** Simplified geometry is fine; hand-drawn geometry is not, because units sit at real coordinates. The dataset is a separate research question tied to the data-licensing decision.
