...
- **Every non-geometric field steps at the phase's instant**: `state`, `formation`, `strength`, caption, wind all hold their phase's value for the whole interval. Strength is a step in v1; the casualties decision may reopen this narrowly.
- **Every phase lists every unit.** The validator rejects a phase that omits a unit another phase has, so a tween always has an origin and a target. Units appearing or disappearing mid-battle is deferred to the decision on how the Combined Fleet splits late at Trafalgar.
