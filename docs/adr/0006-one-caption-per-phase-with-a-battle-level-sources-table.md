...
- **Sources are declared once, at battle level**, keyed by a short id. An entry carries a display `label` and `work` (title, author, edition) as required fields, `url` optional, and a licence field whose contents the data-licensing decision will fix. Unreferenced entries are allowed: a diagram consulted only for positions is still a source and still needs a licence audit.
