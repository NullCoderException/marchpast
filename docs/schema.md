...
- **Views**: named whole visual treatments the viewer picks, built from one engraved system: the Chart plate (default), the Night plate and Atlas. A view is a palette, pens and four modules — `glyph`, `type`, `ground`, `furniture` — over an anatomy fixed across every view (ADR-0021). Chosen with the View chooser and remembered from one visit to the next (ADR-0023); **never authored** — no battle file names a view (ADR-0014, upheld on [#153](https://github.com/NullCoderException/marchpast/issues/153)). (ADR-0014, ADR-0021, ADR-0023, [#47](https://github.com/NullCoderException/marchpast/issues/47), [#58](https://github.com/NullCoderException/marchpast/issues/58))
