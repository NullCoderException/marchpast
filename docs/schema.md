...
*Schema v2, derived from ADR-0001 to ADR-0030, `CONTEXT.md` and the decisions of the v0.2 and v0.3 wayfinder maps ([#36](https://github.com/NullCoderException/marchpast/issues/36), [#121](https://github.com/NullCoderException/marchpast/issues/121)), locked for v0.3 on 2026-09-07 by the handoff ([#141](https://github.com/NullCoderException/marchpast/issues/141)). The source of truth is the code (the types in `src/schema/types.ts`, the licence table in `src/schema/licenses.ts`, the validators `src/schema/validateBattle.ts` and `src/schema/validateMap.ts`) and this document is kept in step with it. The **v0.3 additions in section 0.1** were the one stated exception: they were decided and written here first, and the two build issues that brought the validators to them ([#167](https://github.com/NullCoderException/marchpast/issues/167), [#166](https://github.com/NullCoderException/marchpast/issues/166)) edited the code to this text, never the reverse. Both have landed, so every sentence here now describes the code as it is. A test validates the two JSON examples below against the validators, so the examples show only what the code reads today.*
...
Everything else in v1 stands: positions, the extent's shape and fit, wind, sources and references, licences, moves, the track, the tween rules. A v1 **map** file is a valid v2 map file unchanged. A v1 **battle** file needs `schema_version: 2`, `summary`, `dates` in place of `date`, `sort_date`, `arm` on every unit, and `notes` on every phase (required since 0.1).

### 0.1 What v0.3 adds to v2

Locked on 2026-09-07 by the v0.3 handoff ([#141](https://github.com/NullCoderException/marchpast/issues/141)). Every change is additive: `schema_version` stays `2`, no field is removed, and every v2 file already written is valid unchanged (the four shipped battles gain a `still` flag by choice, not by need). Both validators have caught up: [#167](https://github.com/NullCoderException/marchpast/issues/167) brought the map file's `rampart` kind and its half of the longitude row, and [#166](https://github.com/NullCoderException/marchpast/issues/166) brought the battle file's validator, timeline and player to every other row. This table describes the code, like the rest of the document.

| Change | Where | Decided in |
|---|---|---|
| `arm` gains `aircraft` | 2.3, 2.10 rule 15 | ADR-0024 |
| A unit may be **absent** from a phase: a phase lists the units that exist on the plate at its instant, each unit's phases form one contiguous run, and the sixteen-unit ceiling is counted per level **per phase** | 2.4, 2.9, 2.10 rules 7 and 17 | ADR-0024 |
| `notes` becomes required on every phase; nothing marks a position, a state or a phase as conjectural | 2.4, 2.13 | ADR-0027 |
| A phase may carry `still: true`, at most one per battle: the phase the build renders the library's thumbnail and the battle page's social card from | 2.4, 2.10 rule 18 | ADR-0025, ADR-0028 |
| Longitude is continuous: the extent fixes the frame, `east` may run past 180, and a longitude is bounded against that frame rather than by a flat `-180..180` | 1, 2.10 rule 2, 3.2, 3.3 rule 6 | ADR-0001 (amended 2026-09-07 on [#143](https://github.com/NullCoderException/marchpast/issues/143)) |
| A battle's name may not be a path the build emits at the site's root (`data`, `assets`, `404`, `index`) | 2.10 rule 19 | ADR-0028 |
| Map features gain `rampart`, a built line drawn with its ditch, the teeth on the side it faces | 3.2, 3.3 | ADR-0026 |

Decided and **not** added, so nobody proposes them twice: a `conjectural` flag or a `basis` enum (ADR-0027); a fifth state or an `embarked` state, a fourth formation word, a strike parented to its ship (ADR-0024); a view, an `era` or a `war` field on the battle file (ADR-0014 as amended on [#153](https://github.com/NullCoderException/marchpast/issues/153), ADR-0022); a `still_phase` index (ADR-0025); a `ground` foreign member on the map (ADR-0026); a wrapping extent with `west > east` ([#143](https://github.com/NullCoderException/marchpast/issues/143)).
...
| `arm` | `"infantry"`, `"cavalry"`, `"ship"` or `"aircraft"` | yes | What the unit is made of. Identity, never per-phase state. A closed list kept in code like the licence table: a new arm is a code change that also adds a sign in every view, which is how `aircraft` arrived (ADR-0024). No qualifier: heavy and light, a ship of the line and a frigate, a dive bomber and a torpedo plane, are label and caption matter. A parent carries one arm, that of its main body; there is no `mixed`. A carrier strike is `aircraft`, and its parent is its force, never its ship. |
| `parent` | string | no | The id of the roster unit this one belongs to (a squadron's column). Must name an earlier roster entry with the same `side`. A parent and a child are both full units, each with a snapshot in every phase it exists in (2.9); nothing about a parent is computed from its children. |
...
| `still` | literal `true` | no; at most one phase per battle | Marks the phase whose picture stands for the battle: the build renders the library's thumbnail and the battle page's social card from it, cold, in the default view at the coarsest level (ADR-0025, ADR-0028). When no phase carries it the build takes the earliest phase whose count of `engaged` units over the whole roster is the battle's maximum. The flag rides its phase, so inserting a phase before it cannot move the picture. |
| `units` | UnitSnapshot[] | yes | One snapshot for every roster unit that exists on the plate at this instant, parents and children alike, no duplicates and no id off the roster. A roster unit missing from a phase is **absent** (2.9): it does not exist on the plate then, and the phase says nothing else about it. |
...
**Showing a level draws every unit at that depth plus every shallower unit that has no children.** A unit the author chose not to split stands for itself at every finer level, so nothing vanishes from the plate. Every unit that exists at a phase's instant, drawn or not, has a snapshot in it; a parent keeps its own position, heading, formation, state, strength and moves in phases where only its children are drawn, and nothing rolls up or is checked between levels: each level is a self-contained picture authored from the sources.

**Absence.** A phase omits a roster unit only when the unit does not exist on the plate at that instant — not yet launched, already recovered, not yet arrived, gone — never because its position is unknown or lies off the extent (ADR-0024). A unit's phases form **one contiguous run** of at least one phase; a second sortie is a second unit. The player draws a unit across an interval only when it has a snapshot at both ends, so it appears at the instant of its first phase and is gone after the instant of its last, with no fade. Which units a level *can* draw is a property of the roster and never changes with the phase; presence filters that set per phase, so a parent never pops onto a finer level in the hours its children are away, and a parent that must stay visible there is given a permanent childless child, as Midway's `midway-air` is.
...
**Unit count.** More than sixteen units drawn at one level is a validation error (rule 17), the label placement proof's break point made a rule. Twelve is the guideline: twelve carry full labels, sixteen still read with a few short names, twenty break. Counted per level **per phase**, never per roster: Trafalgar at three columns and their squadrons is drawn as three or as the squadrons, never both, and Midway's strikes count only in the phases they are airborne, so the question is how many at 07:55, not how many exist all day (ADR-0024).
...
7. Every snapshot names a roster unit, no phase names a unit twice, every roster unit appears in at least one phase, and each unit's phases form one contiguous run (ADR-0024). A phase need not list every unit.
...
15. `arm` is on the allowlist (`infantry`, `cavalry`, `ship`, `aircraft`).
...
17. `levels` is present if and only if some unit has a `parent`; its length equals the tree's depth (the deepest unit's depth plus one); and **no level draws more than sixteen units in any phase**, counting at each depth `n` the units at depth `n` plus the shallower units with no children, intersected with the phase's snapshots (ADR-0024).
18. `still`, when present, is `true`, and at most one phase carries it (ADR-0025).
19. The battle's **name**, the file stem under `data/battles/`, is not `data`, `assets`, `404` or `index`, the paths the build emits at the site's root (ADR-0028). The validator sees a document and not a filename, so `scripts/validate.ts` and the library build apply this one.

What the validator does not check: that positions lie inside the extent (a move head may not; a unit usually does), the direction of a heading tween, caption length, that `sort_date` agrees with `dates[0]`, that a unit's absence from a phase is non-existence rather than ignorance (2.13), or any consistency between `state`, `strength` and `moves`, between a parent and its children, or between `arm` and `formation`.
...
- **Loading** lands paused on the first phase, in the view the viewer left last time or else the Chart plate, at the coarsest level, with no unit card open. The view is remembered from one visit to the next (ADR-0023); the level, the speed multiplier, the clock and the pinned card are not, and nothing is carried on the URL.
- **A unit exists only inside its run.** The timeline computes a picture for a unit only between two phases that both hold a snapshot of it; outside its run it is not drawn, not labelled, not in the muster and not on a card (ADR-0024).
...
- **A strike aboard its carrier is `column` on the carrier's own heading**, at her position, so its aeroplanes lie in file along her and overprint the hull; recovery is caption matter, as anchoring is ([#140](https://github.com/NullCoderException/marchpast/issues/140), ADR-0024).
...
- **Views**: named whole visual treatments of the picture and the player's surface that the viewer picks: the Chart plate (default), the Night plate, Atlas and the Staff map. A view is a palette, pens and **five modules** — `glyph`, `type`, `ground`, `furniture`, `moves` — over an **anatomy** fixed across every view: what the picture shows, where each thing sits and what it means (ADR-0021; the fifth slot cut against the staff map's drawing on [#139](https://github.com/NullCoderException/marchpast/issues/139)). The three engraved views share one aesthetic and the staff map is a second; nothing in the code counts them. Chosen with the View chooser and **remembered** from one visit to the next; reaching the control strip, the details panel and the page ground (ADR-0023, ADR-0030); **never authored** — no battle file names a view (ADR-0014, upheld on [#153](https://github.com/NullCoderException/marchpast/issues/153)) — and never chosen by an operating-system preference (ADR-0029). (ADR-0014, ADR-0021, ADR-0023, [#47](https://github.com/NullCoderException/marchpast/issues/47), [#58](https://github.com/NullCoderException/marchpast/issues/58), [#139](https://github.com/NullCoderException/marchpast/issues/139))
- **Glyph and signs**: eight signs per unit arranged by formation at a fixed 72px plate length on the engraved views; a sign per arm per view (the plate's chevron tick, rank bar, barred rank bar and aeroplane in plan; Atlas's block with crossed, single or no diagonal and the same aeroplane; the staff map's frame with crossed diagonals, one diagonal, a hull in plan and the symbol tradition's stroked arrowhead), a view drawing an arm its own way because what every view shares is the distinction, never the drawing ([#140](https://github.com/NullCoderException/marchpast/issues/140)); shown signs follow strength, a `mass` losing its rear rank first; the engaged mark is Billow on the engraved views, outlined puffs drifting to the lee flank under the wind, laid down for every unit before any unit's body, and on the staff map a serration along the unit's own front inside its footprint, which lays nothing down first. (ADR-0014, ADR-0015, ADR-0016, ADR-0024, [#139](https://github.com/NullCoderException/marchpast/issues/139), [#140](https://github.com/NullCoderException/marchpast/issues/140))
...
- **Map drawing**: land in a darker paper with an ink coastline; relief as contours weighted by level, every fifth numbered; a shoal as a dotted edge with a fine stipple; a river as two banks clipped to the land; a rampart as a line with its ditch teeth on the side it faces (engraved, a fine line with 4.5px teeth as a siege plan cuts it; atlas, one heavier line; the staff map, an obstacle with cross ticks both sides); a work as a bastioned-square sign with its name in small capitals; a place as a dot with its name in italic. Drawn in that order, land first, the ramparts among the ground, and the named things last. The index contours are every fifth level, found from the levels the file carries and never flagged in it. **Each view draws its own ground** through its `ground` hand, and every treatment comes off the contour polylines and their levels alone, no raster (ADR-0021, [#138](https://github.com/NullCoderException/marchpast/issues/138)): the chart plate, contours weighted by level with the numerals in a column at the foot; the night plate, the same contours lit from one side, the shaded side thinning into the ground; Atlas, a hypsometric ramp in five bands above 30, 50, 100, 150 and 200 m under faint index contours, a numeral knocking out to the band it stands on; the staff map, flat steps of colour, a kilometre graticule under the units, the height set into the line, a body of water with a shallow tint in place of the shoal stipple. What stays shared is the order and the frame — land first, the treatment, water, the ramparts among the ground, the named things last, clipped to the extent — and the ground is drawn once to an offscreen buffer per view, extent and device pixel ratio, not per frame. On a plate with contours or a graticule the furniture sits on paper panels and the scale bar carries the interval. (ADR-0012, ADR-0014, ADR-0021, [#62](https://github.com/NullCoderException/marchpast/issues/62), [#138](https://github.com/NullCoderException/marchpast/issues/138))
- **Furniture**: compass rose with the wind arrow and text (top-left), plate title (top-right), scale bar in `scale_unit` and the always-on legend (bottom-left), the map file's `attribution` credit line (bottom-right). North is always up. Which pieces there are and which corner each anchors to is anatomy; how each is drawn is the view's `furniture` hand — the staff map's rose is a plain needle with no declination diagram, its wind a station barb, its scale bar alternating blocks, its legend a marginal key, its border a neat line (ADR-0021, [#139](https://github.com/NullCoderException/marchpast/issues/139)). (ADR-0005, ADR-0009, ADR-0021, #58)
- **Legend**: each side's colour and `side` name, the four state glyphs, the three motion styles (track, intent, detachment) sampled from the view's own `moves` hand, one row per arm on the roster at the level shown when there are two or more (so a row does not blink out in a phase where its arm is absent, ADR-0024), and a numeral key for any unit showing one this frame. No row for formation or level. (ADR-0015, ADR-0016, ADR-0017, ADR-0024, #39)
...
- **Controls**: at the strip's head `← All battles`, the one link that leaves (ADR-0030); then play/pause, the phase-segmented scrubber with an `HH:MM` readout, next/previous phase, the 0.5x/1x/2x/4x speed multiplier, the View chooser, the Level chooser, the Details toggle, and last the Picker, which leaves the battle rather than moving about inside it. The three choosers are native `<select>`s with only the caret drawn in the view's ink. A phase jump **cuts** where playback tweens, so the jump buttons and the caption band are the unmoving way through a battle, a stated affordance and not an accident (ADR-0029). Only the view is stored. (#13, #47, ADR-0017, ADR-0011, ADR-0023, ADR-0030)
- **Surface**: the control strip, the details panel and the page ground take their tokens from the view's palette — the ink, the ground from the paper, the fill as ink at 8% over the ground, the rule as the least alpha from 0.55 that clears 3:1, the page edge from the letterbox, the face from `type` — written once per switch and conformance-tested for contrast over every view (ADR-0023, ADR-0030). The site reads no operating-system preference and the surface declines forced colours with `forced-color-adjust: none` (ADR-0029). The library is never themed.
- **Muster**: a visually hidden listbox after the canvas holding the units the plate is drawing this phase at this level, one tab stop with roving focus; focus opens the unit's card and Enter pins it; the plate is `role="img"`, named by the battle and described by a live announcer that reads the phase on change. ([#130](https://github.com/NullCoderException/marchpast/issues/130))
- **Library, battle pages and Picker**: the site's root lists every battle oldest first down a **chronology rail** that writes the years between consecutive battles and never groups them (ADR-0022), each entry a **still** of the phase the battle marks (the thumbnail at 288 × 182, rendered by the build with the player's renderer in the default view, ADR-0025), its `title`, `dates[0]`, its side names in their inks and its `summary`; generated by the build from the battle files into `data/index.json` (name, title, date, sort date, summary, side names), `data/stills/<name>.png` and `data/cards/<name>.png`. A battle plays at **`/<name>/`**, a page the build emits with that battle's own title, description and social card (ADR-0028); `?battle=<name>` is read as a legacy form and never emitted; a path the site does not hold answers with `404.html`. There is no default battle. Every file in `data/battles/` is a battle the library offers, so only battles go there; a fixture, which is battle-shaped but exists to exercise the code, lives in `src/app/fixtures.ts` and plays at `?fixture=<name>`, outside the library. (ADR-0011, ADR-0022, ADR-0025, ADR-0028)
...
- **Projection**: Web Mercator. Never a field. **Palette, face, glyph sizes**: the view's inks by side order, the view's face (IM Fell English on the engraved views, Archivo at wdth 75 on the staff map), a fixed glyph length.
...
| `summary`; the Library, `data/index.json`, the Picker (its `?battle=` clause superseded by ADR-0028) | [ADR-0011](adr/0011-the-library-is-built-from-the-battle-files-and-the-url-names-a-battle-by-query.md) |
...
| Views: the anatomy fixed, a view's five hands, a second aesthetic | [ADR-0021](adr/0021-the-anatomy-is-fixed-and-a-view-owns-how-it-is-drawn.md) |
| The library as a chronology; the rail; the interval from `sort_date` | [ADR-0022](adr/0022-the-library-is-a-chronology-not-a-grouping.md) |
| The view reaching the surface and remembered; the surface's tokens; `← All battles` | [ADR-0023](adr/0023-a-view-reaches-the-player-surface-and-is-remembered.md), [ADR-0030](adr/0030-the-surface-derives-its-tones-from-the-ink-and-the-strip-carries-the-way-back.md) |
| `aircraft`; absence; the ceiling per level per phase | [ADR-0024](adr/0024-aircraft-are-an-arm-and-a-unit-may-be-absent-from-a-phase.md) |
| `still`; the thumbnail and the still pipeline | [ADR-0025](adr/0025-the-library-shows-a-still-rendered-by-the-build-from-the-phase-a-battle-marks.md) |
| A page per battle at `/<name>/`; the social card; reserved names | [ADR-0028](adr/0028-a-battle-is-a-page-at-its-own-path-and-its-card-is-its-own-still.md) |
| No operating-system preference read; forced colours declined | [ADR-0029](adr/0029-the-site-reads-no-operating-system-preference-and-the-surface-declines-forced-colours.md) |
| Terrain per view, the staff-map view, the aircraft sign, the muster, the brand (no ADR) | [#138](https://github.com/NullCoderException/marchpast/issues/138), [#139](https://github.com/NullCoderException/marchpast/issues/139), [#140](https://github.com/NullCoderException/marchpast/issues/140), [#130](https://github.com/NullCoderException/marchpast/issues/130), [#135](https://github.com/NullCoderException/marchpast/issues/135) |
