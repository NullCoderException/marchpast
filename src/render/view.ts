/**
 * What a view *is*: the seam itself (supersedes ADR-0009's single chart plate).
 * The views there are live in `views.ts`, so a glyph module can depend on this
 * file without depending on the views built from it.
 *
...
/** The view's key: what player state holds and the URL carries. */
...
export interface GlyphRequest {
  ...
  /** The view's materials, so a glyph can fill with the paper it sits on. */
  palette: Palette;
}
...
export interface View {
  id: ViewId;
  /** What a picker shows. Never drawn on the plate. */
  name: string;
  palette: Palette;
  pens: Pens;
  glyph: Glyph;
