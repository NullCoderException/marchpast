 ...
 *
 * The unit glyph is *not* here any more: it is the one pass a view replaces,
 * so it lives in `glyphs/` and reaches the passes through `view.glyph`.
 */
 ...
 * weight and head; the colour comes from the palette (ink) or the side ink, so
 * a view never restates a colour it has already declared.
 */
