...
function dimensions(run: number, taper = STAFF_TAPERS.intent): { tail: number; shoulder: number; head: number; headLength: number } {
  const [tailCorner, shoulderCorner, headCorner, point] = taperPolygon(FROM, east(run), taper) as Point[];
  return {
    tail: tailCorner!.y,
...
describe("taperPolygon", () => {
  it("closes seven points: the tail, the shoulder, the head's barbs and the point", () => {
    const points = taperPolygon(FROM, east(200), STAFF_TAPERS.intent);
  ...
  it("draws its full dimensions at the run they were set for and above", () => {
    // At 150 px and beyond the scale is 1, so the four dimensions are the
    // table's own and the head is nowhere near 45% of the run.
  ...
  it("draws a detachment broader than an intent at every run", () => {
    for (const run of [30, 150, 400]) {
      const intent = dimensions(run, STAFF_TAPERS.intent);
      const detachment = dimensions(run, STAFF_TAPERS.detachment);
  ...
  it("draws nothing at all for a move that goes nowhere, which names no direction", () => {
    expect(taperPolygon(FROM, { x: 0, y: 0 }, STAFF_TAPERS.intent)).toEqual([]);
  });
});
