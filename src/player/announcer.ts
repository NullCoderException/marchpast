 ...
 * would talk over everything else on the page sixty times a second. The phase
 * count is the orientation a sighted viewer reads off the scrubber's ticks and
 * nothing else carries, which is why the bar's `aria-valuetext` takes the same
 * `phaseCount` rather than a wording of its own.
 *
...
/** Which phase of how many, counted from one: `Phase 4 of 12`. */
export function phaseCount(picture: Picture, phases: number): string {
...
export function announcement(picture: Picture, phases: number): string {
  return `${phaseCount(picture, phases)}, ${picture.phase.t} — ${picture.phase.label}`;
}
