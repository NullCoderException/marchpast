...
import { announcement, phasePosition } from "./announcer.ts";
...
describe("phasePosition", () => {
  it("counts the phase the instant falls in, from one", () => {
    expect(phasePosition(pictureAt(TEST_BATTLE, clock("10:00")), TEST_BATTLE.phases.length)).toBe("Phase 1 of 3");
    expect(phasePosition(pictureAt(TEST_BATTLE, clock("10:15")), TEST_BATTLE.phases.length)).toBe("Phase 2 of 3");
  });
  ...
  it("counts a phase still running, not the instant it began on", () => {
    ...
    expect(phasePosition(pictureAt(ABSENCE_BATTLE, clock("12:30")), ABSENCE_BATTLE.phases.length)).toBe("Phase 3 of 6");
  });
});
