...
import { buildLibrary, libraryEntry, sideNames } from "./library.ts";
...
describe("the entry one battle file makes", () => {
  it("carries the name, title, first date, sort date, summary and sides", () => {
    expect(libraryEntry("trafalgar", MINIMAL_BATTLE)).toEqual({
      name: "trafalgar",
  ...
  it("takes the first day's date, not the last", () => {
    ...
    expect(libraryEntry("nile", battle).date).toBe("1 August 1798");
  });
});
