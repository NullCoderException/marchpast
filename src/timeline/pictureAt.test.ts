...
describe("pictureAt: geometry tweens", () => {
  it("puts a unit at the arithmetic mean of the two positions at the midpoint of the interval", () => {
    const picture = pictureAt(TEST_BATTLE, at("10:05"));
...
describe("pictureAt: everything else steps", () => {
  it("does not tween strength", () => {
    ...
    expect(pictureAt(TEST_BATTLE, at("10:10")).units.find((u) => u.id === "alpha")?.strength).toBe(0.33);
  });
  ...
  it("steps wind, caption, label, moves, formation and state at the phase's t", () => {
    const before = pictureAt(TEST_BATTLE, at("10:09:59"));
    ...
    const on = pictureAt(TEST_BATTLE, at("10:10"));
  ...
  it("carries the phase's references, and its notes only when it has them", () => {
    expect(pictureAt(TEST_BATTLE, at("10:05")).references).toEqual([{ source: "invented", locator: "p. 1" }]);
    expect(pictureAt(TEST_BATTLE, at("10:05")).notes).toBe("A note only the first phase has.");
    expect(pictureAt(TEST_BATTLE, at("10:15")).notes).toBeUndefined();
  });
...
describe("pictureAt: the last phase holds", () => {
  ...
  it("still shows it at end itself", () => {
    expect(pictureAt(TEST_BATTLE, at("10:30")).phaseIndex).toBe(2);
    expect(pictureAt(TEST_BATTLE, at("10:30")).clock).toBe(at("10:30"));
  });

  it("hands out positions the renderer cannot use to edit the battle", () => {
    const picture = pictureAt(TEST_BATTLE, at("10:29:59"));
...
describe("pictureAt: the clock", () => {
  it("clamps an instant before the first phase to the first phase's picture", () => {
    const picture = pictureAt(TEST_BATTLE, at("09:00"));
    expect(picture.phaseIndex).toBe(0);
    expect(picture.clock).toBe(at("10:00"));
  ...
  it("clamps an instant past end to end", () => {
    expect(pictureAt(TEST_BATTLE, at("23:00")).clock).toBe(at("10:30"));
  });

  it("lists every roster unit in roster order", () => {
    expect(pictureAt(TEST_BATTLE, at("10:05")).units.map((u) => u.id)).toEqual(["alpha", "beta"]);
  });
});
